...
export default async function DashboardSayaPage() {
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      <div>
        <h1 className="text-2xl font-bold">Konten Saya</h1>
        <p className="text-muted-foreground text-sm mt-1">Kelola iklan, UMKM, dan lowongan yang kamu posting</p>
      </div>
              ...
              {lokerList.map((item) => (
                <li key={item.id} className="flex items-center gap-4 px-4 py-3">
                  ...
                  <PostActions
                    type="loker"
                    id={item.id}
                    extraActions={item.isActive ? [{ label: "Tutup Lowongan", action: "close" }] : [{ label: "Buka Kembali", action: "reopen" }]}
                  />
                </li>
              ))}
            </ul>
