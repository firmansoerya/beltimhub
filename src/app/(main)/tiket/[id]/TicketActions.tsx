...
async function generateTicketCanvas(data: TicketData): Promise<HTMLCanvasElement> {
  await document.fonts.ready;

  // Hitung tinggi dinamis berdasarkan jumlah field
  ...
  ctx.fillText(data.eventLocation, W / 2, 110);

  // QR code centered
  const qrImg = new Image();
  qrImg.src = data.qrCodeData;
  ...
  // Kumpulkan field yang akan ditampilkan
  ...
  // Jika jumlah field ganjil, tetap naikkan baris
...
export function DownloadTicketButton({ data }: { data: TicketData }) {
  ...
  async function handleDownload() {
    ...
    try {
      ...
      link.download = `tiket-${data.ticketCode}.png`;
  ...
  return (
    <button
      ...
      disabled={loading}
      className="w-full flex items-center justify-center gap-2 rounded-lg px-4 py-2 border border-border text-sm font-medium hover:bg-accent transition-colors disabled:opacity-60"
    >
      <Download className="w-4 h-4" />
      {loading ? "Menyiapkan..." : "Download Tiket"}
    </button>
  );
}
